...
struct ImDrawListSharedData;        // Data shared among multiple draw lists (typically owned by parent ImGui myContext, but you may create one yourself)
...
struct ImGuiContext;                // Dear ImGui myContext (opaque structure, unless including imgui_internal.h)
...
namespace ImGui
{
    ...
    // - Each myContext create its own ImFontAtlas by default. You may instance one yourself and pass it to CreateContext() to share a font atlas between contexts.
    // - DLL users: heaps and globals are not shared across DLL boundaries! You will need to call SetCurrentContext() + SetAllocatorFunctions()
    //   for each static/DLL boundary you are calling from. Read "Context and Memory Allocators" section of imgui.cpp for details.
    IMGUI_API ImGuiContext* CreateContext(ImFontAtlas* shared_font_atlas = NULL);
    IMGUI_API void          DestroyContext(ImGuiContext* ctx = NULL);   // NULL = destroy current myContext
    ...
    IMGUI_API void          NewLine();                                                      // undo a SameLine() or force a new line when in a horizontal-layout myContext.
    ...
    //  - They are convenient to easily create myContext menus, hence the name.
    ...
    //   Headers are required to perform: reordering, sorting, and opening the myContext menu.
    //   The myContext menu can also be made available in columns body using ImGuiTableFlags_ContextMenuInBody.
    // - You may manually submit headers using TableNextRow() + TableHeader() calls, but this is only useful in
    //   some advanced use cases (e.g. adding custom widgets in header row).
    // - Use TableSetupScrollFreeze() to lock columns/rows so they stay visible when scrolled.
    IMGUI_API void          TableSetupColumn(const char* label, ImGuiTableColumnFlags flags = 0, float init_width_or_weight = 0.0f, ImGuiID user_id = 0);
    IMGUI_API void          TableSetupScrollFreeze(int cols, int rows);         // lock columns/rows so they stay visible when scrolled.
    IMGUI_API void          TableHeadersRow();                                  // submit all headers cells based on data provided to TableSetupColumn() + submit myContext menu
    ...
    IMGUI_API void                  TableSetColumnEnabled(int column_n, bool v);// change user accessible enabled/disabled state of a column. Set to false to hide the column. User can use the myContext menu to change this themselves (right-click in headers, or right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
    ...
    // Memory Allocators
    // - Those functions are not reliant on the current myContext.
    // - DLL users: heaps and globals are not shared across DLL boundaries! You will need to call SetCurrentContext() + SetAllocatorFunctions()
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_UnsavedDocument        = 1 << 20,  // Display a dot next to the title. When used in a tab/docking myContext, tab is selected when clicking the X + closure is not assumed (will wait for user to stop submitting the tab). Otherwise closure is assumed when pressing the X, so if you keep submitting the tab may reappear at end of tab bar.
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_Hideable                   = 1 << 2,   // Enable hiding/disabling columns in myContext menu.
    ImGuiTableFlags_Sortable                   = 1 << 3,   // Enable sorting. Call TableGetSortSpecs() to obtain sort specs. Also see ImGuiTableFlags_SortMulti and ImGuiTableFlags_SortTristate.
    ImGuiTableFlags_NoSavedSettings            = 1 << 4,   // Disable persisting columns order, width and sort settings in the .ini file.
    ImGuiTableFlags_ContextMenuInBody          = 1 << 5,   // Right-click on columns body/contents will display table myContext menu. By default it is available in TableHeadersRow().
    // Decorations
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_Disabled              = 1 << 0,   // Overriding/master disable flag: hide column, won't show in myContext menu (unlike calling TableSetColumnEnabled() which manipulates the user accessible state)
    ...
    ImGuiTableColumnFlags_NoHeaderLabel         = 1 << 12,  // TableHeadersRow() will not submit label for this column. Convenient for some small columns. Name will still appear in myContext menu.
...
struct ImGuiIO
{
    ...
    int         MetricsActiveAllocations;           // Number of active allocations, updated by MemAlloc/MemFree based on current myContext. May be off if you have multiple imgui contexts.
...
struct ImGuiPayload
{
    ...
    ImGuiPayload()  { ClearRenderTarget(); }
    void ClearRenderTarget()    { SourceId = SourceParentId = 0; Data = NULL; DataSize = 0; memset(DataType, 0, sizeof(DataType)); DataFrameCount = -1; Preview = Delivery = false; }
...
struct ImGuiTextFilter
{
    ...
    void                ClearRenderTarget()          { InputBuf[0] = 0; Build(); }
...
struct ImGuiStorage
{
    ...
    void                ClearRenderTarget() { Data.clear(); }
...
struct ImDrawListSplitter
{
    ...
    inline void                 ClearRenderTarget() { _Current = 0; _Count = 1; } // Do not clear Channels[] so our allocations are reused next frame
...
struct ImDrawList
{
    ...
    ImDrawListSharedData*   _Data;              // Pointer to shared draw data (you can use ImGui::GetDrawListSharedData() to get the one from current ImGui myContext)
...
struct ImDrawData
{
    ...
    // Functions
    ImDrawData()    { ClearRenderTarget(); }
    void ClearRenderTarget()    { memset(this, 0, sizeof(*this)); }     // The ImDrawList are owned by ImGuiContext!
...
struct ImFontGlyphRangesBuilder
{
    ...
    ImFontGlyphRangesBuilder()              { ClearRenderTarget(); }
    inline void     ClearRenderTarget()                 { int size_in_bytes = (IM_UNICODE_CODEPOINT_MAX + 1) / 8; UsedChars.resize(size_in_bytes / (int)sizeof(ImU32)); memset(UsedChars.Data, 0, (size_t)size_in_bytes); }
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              ClearRenderTarget();                    // Clear all input and output.
