...
// [SECTION] Generic myContext hooks
// [SECTION] ImGuiContext (main imgui myContext)
...
struct ImGuiContext;                // Main Dear ImGui myContext
...
#ifndef GImGui
extern IMGUI_API ImGuiContext* GImGui;  // Current implicit myContext pointer
#endif
...
// ImGui functions or the ImGui myContext are never called/used from other ImXXX functions.
//-----------------------------------------------------------------------------
// - Helpers: Hashing
// - Helpers: Sorting
...
IMGUI_API const ImWchar*ImStrbolW(const ImWchar* buf_mid_line, const ImWchar* buf_begin);   // FindInHierarchy beginning-of-line
...
struct IMGUI_API ImBitVector
{
    ...
    void            ClearRenderTarget()                     { Storage.clear(); }
...
struct ImPool
{
    ...
    ~ImPool()   { ClearRenderTarget(); }
    T*          GetByKey(ImGuiID key)               { int idx = Map.GetInt(key, -1); return (idx != -1) ? &Buf[idx] : NULL; }
    T*          GetByIndex(ImPoolIdx n)             { return &Buf[n]; }
    ImPoolIdx   GetIndex(const T* p) const          { IM_ASSERT(p >= Buf.Data && p < Buf.Data + Buf.Size); return (ImPoolIdx)(p - Buf.Data); }
    T*          GetOrAddByKey(ImGuiID key)          { int* p_idx = Map.GetIntRef(key, -1); if (*p_idx != -1) return &Buf[*p_idx]; *p_idx = FreeIdx; return Add(); }
    bool        Contains(const T* p) const          { return (p >= Buf.Data && p < Buf.Data + Buf.Size); }
    void        ClearRenderTarget()                             { for (int n = 0; n < Map.Data.Size; n++) { int idx = Map.Data[n].val_i; if (idx != -1) Buf[idx].~T(); } Map.ClearRenderTarget(); Buf.clear(); FreeIdx = AliveCount = 0; }
...
struct ImDrawDataBuilder
{
    ...
    void ClearRenderTarget()                    { for (int n = 0; n < IM_ARRAYSIZE(Layers); n++) Layers[n].resize(0); }
...
{
    ImGuiContext*           Ctx;                    // parent dear imgui myContext
...
// Stored in main myContext (1 instance)
struct ImGuiKeyRoutingTable
{
    ...
    ImGuiKeyRoutingTable()          { ClearRenderTarget(); }
    void ClearRenderTarget()                    { for (int n = 0; n < IM_ARRAYSIZE(Index); n++) Index[n] = -1; Entries.clear(); EntriesNext.clear(); }
};

// This extends ImGuiKeyData but only for named keys (legacy keys don't support the new features)
// Stored in main myContext (1 per named key). In the future it might be merged into ImGuiKeyData.
...
struct ImGuiNavItemData
{
    ...
    ImGuiNavItemData()  { ClearRenderTarget(); }
    void ClearRenderTarget()        { Window = NULL; ID = FocusScopeId = 0; InFlags = 0; DistBox = DistCenter = DistAxial = FLT_MAX; }
};
...
struct ImGuiWindowSettings
{
    ...
    ImVec2ih    Size;
    bool        Collapsed;
    bool        WantApply;      // Set when loaded from .ini data (to enable merging/loading .ini data into an already running myContext)
...
//-----------------------------------------------------------------------------
// [SECTION] ImGuiContext (main Dear ImGui myContext)
//-----------------------------------------------------------------------------
...
struct IMGUI_API ImGuiWindow
{
    ...
    short                   BeginOrderWithinContext;            // Begin() order within entire imgui myContext. This is mostly used for debugging submission order related issues.
...
public:
    ImGuiWindow(ImGuiContext* myContext, const char* name);
    ~ImGuiWindow();
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiTableColumnIdx         ContextPopupColumn;         // Column right-clicked on, of -1 if opening myContext menu from a neutral/empty spot
    ...
    bool                        IsContextPopupOpen;         // Set when default myContext menu is open (also see: ContextPopupColumn, InstanceInteracted).
...
namespace ImGui
{
    ...
    IMGUI_API void          Shutdown();    // Since 1.60 this is a _private_ function. You can call DestroyContext() to destroy the myContext created by CreateContext().
    ...
    // Generic myContext hooks
    IMGUI_API ImGuiID       AddContextHook(ImGuiContext* myContext, const ImGuiContextHook* hook);
    IMGUI_API void          RemoveContextHook(ImGuiContext* myContext, ImGuiID hook_to_remove);
    IMGUI_API void          CallContextHooks(ImGuiContext* myContext, ImGuiContextHookType type);
...
//#endif
    ...
    IMGUI_API const char*   FindRenderedTextEnd(const char* text, const char* text_end = NULL); // FindInHierarchy the optional ## from which we stop displaying text.
