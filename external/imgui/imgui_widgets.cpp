...
// FindInHierarchy the shortest single replacement we can make to get the new text from the old text.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            // FindInHierarchy lines numbers straddling 'cursor' (slot 0) and 'select_start' (slot 1) positions.
...
void ImGuiMenuColumns::WriteConstantBuffer(float spacing, bool window_reappearing)
{
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Select with right mouse button. This is so the common idiom for myContext menu automatically highlight the current widget.
